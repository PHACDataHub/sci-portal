...
const fetchProjectBudgets = async (
  ...
  try {
    const response = await fetch(`${apiEndpoint}/api/budget-usage/usages`, {
      headers,
    });
    if (!response.ok) {
    ...
    console.error(
      'ERROR_FETCHING_BUDGETS: An error occurred while fetching budgets',
      error,
    );
    return [];
  }
...
const DataLoaderContext = createContext<DataLoaderContextType | undefined>(
  undefined,
);
