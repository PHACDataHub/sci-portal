...
describe('getUnqueuedThresholdExceededEvents', () => {
  ...
  test('Given THRESHOLD_EXCEEDED events without corresponding QUEUED_BUDGET_ALERT_EMAILS events it should return them in an array', () => {
    const events = [
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.50 })),
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.75 })),
    ];
  ...
  test('Given THRESHOLD_EXCEEDED events without corresponding QUEUED_BUDGET_ALERT_EMAILS events it should return them in an array', () => {
    const events = [
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.50 })),
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.75 })),
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 1.00 })),
    ];
    const actual = getUnqueuedThresholdExceededEvents(events);
    expect(actual).toEqual(events);
  });
  ...
  test('Given THRESHOLD_EXCEEDED events without corresponding QUEUED_BUDGET_ALERT_EMAILS events it should return them in an array', () => {
    const events = [
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.50 })),
      createQueuedBudgetAlertEmailsEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.50 }), []),
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 0.75 })),
      createThresholdExceededEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 1.00 })),
      createQueuedBudgetAlertEmailsEvent(/** @type {any} */({ projectId: 'test-project', alertThresholdExceeded: 1.00 }), []),
    ];
...
describe('getQueuedBudgetAlertEmails', () => {
  ...
  test('Given an email has been queued but no emails have been sent it should return an item for each recipient', () => {
    ...
    const events = [
      createQueuedBudgetAlertEmailsEvent(/** @type {any} */({ projectId, costIntervalStart, alertThresholdExceeded }),
  ...
  test('Given an email has been queued but no emails have been sent it should return an item for each recipient', () => {
    const events = [
      createQueuedBudgetAlertEmailsEvent(/** @type {any} */({
  ...
  test('Given an email has been queued and is sending it should not return that recipient', () => {
    ...
    const events = [
      ...
      createSendingBudgetAlertEmailEvent(/** @type {any} */({
        projectId,
        costIntervalStart,
        alertThresholdExceeded,
        recipient: 'jane.doe@gcp.hc-sc.gc.ca'
